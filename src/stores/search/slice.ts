...
const search = createSlice({
  ...
  extraReducers(builder) {
    ...
    builder.addCase(saveWord.fulfilled, (state, { payload }: any) => {
      ...
      const dataIndex = cloneData.findIndex((item: Record<string, any>) => item.content === content)
      if (dataIndex !== -1) {
        const dataItem: Record<string, any> = { ...(cloneData[dataIndex] as Record<string, any>) }
        const detailData = [...dataItem.detail]

        const itemIndex = detailData.findIndex((detailItem: Record<string, any>) => detailItem.id === wordId)

        if (itemIndex !== -1) {
          detailData.splice(itemIndex, 1, { ...detailData[itemIndex], savedFlg: true})
        }
