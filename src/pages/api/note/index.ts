...
export default async function handler(
  ...
  try {
    ...
    const {
      level: levelQueryString,
      limit: limitQueryString,
      afterAt,
      keyword,
    } = req.query;

    const level = /^[1-5]$/.test(levelQueryString as string)
      ? Number(levelQueryString)
      : null;
    ...
    let querySnapshot = (coll: Query<unknown>, ...props: any) =>
      query(coll, where("rate", ">=", 5), ...props);

    switch (level) {
      case 1: {
        querySnapshot = (coll: Query<unknown>, ...props) =>
          query(coll, where("rate", "<=", 1), ...props);
        break;
      }
      case 2: {
        querySnapshot = (coll: Query<unknown>, ...props) =>
      ...
      case 4: {
        querySnapshot = (coll: Query<unknown>, ...props) =>
          query(coll, where("rate", ">", 3.6), where("rate", "<", 5), ...props);
        break;
      }
    }

    const limitValue = Number(limitQueryString as string) || 30;

    const options: any[] = [
      where("user", "==", userQuery),
      orderBy("rate"),
      limit(limitValue > 30 || limitValue <= 0 ? 30 : limitValue),
    ];
    ...
    const wordStorages = await getWordStorageByLevel(querySnapshot, ...options);
    ...
    const wordStorageResponseData = JSON.parse(
      JSON.stringify(
        wordStorages.map(({ id, rate, vocabulary_id }: any) => {
          return {
            id,
            rate,
            calcRate: level,
